...
namespace Aplicacao.Aplicacoes
{
    ...
    {
        INoticia _INoticia;
        IServicoNoticia _IServicoNotica;

        public AplicacaoNoticia(INoticia iNoticia, IServicoNoticia iServicoNotica)
        {
            _INoticia = iNoticia;
            _IServicoNotica = iServicoNotica;
        }

        public async Task AdicionaNoticia(Noticia noticia)
        {
            await _IServicoNotica.AdicionaNoticia(noticia);
        }
        public async Task<List<Noticia>> ListarNoticiasAtivas()
        {
            return await _IServicoNotica.ListarNoticiasAtivas();
        }

        public async Task AtualizaNoticia(Noticia noticia)
        {
            await _IServicoNotica.AtualizaNoticia(noticia);
        }
        ...
        public async Task<Noticia> PuscarPorId(int Id)
        {
